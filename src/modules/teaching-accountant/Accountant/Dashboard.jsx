...
const quickActions = [
  { icon: "bi bi-person-workspace", label: "Add Staff", to: "/teaching-accountant/admission/teaching" },
  { icon: "bi bi-cash-coin", label: "Pay Salary", to: "/teaching-accountant/accounts/staff/teaching" },
  { icon: "bi bi-receipt", label: "Add Expense", to: "/teaching-accountant/accounts/expenses" },
  { icon: "bi bi-megaphone", label: "Create Notice", to: "/teaching-accountant/notices" },
  { icon: "bi bi-file-earmark-bar-graph", label: "Generate Report", to: "/teaching-accountant/accounts/reports" },
  { icon: "bi bi-calendar2-check", label: "Mark Attendance", to: "/teaching-accountant/attendance" },
];
...
export default function Dashboard() {
  ...
  const totalStaff = teacherRows.length;

  const sortedStaff = useMemo(() => [...teacherRows].sort((a, b) => a.name.localeCompare(b.name)), []);
  const totalPages = Math.ceil(sortedStaff.length / PAGE_SIZE);
  const pageRows = sortedStaff.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  return (
    <div>
      <PageHeader title="Teaching Accountant" subtitle="Amit Group of Schools — welcome back, Teaching Accountant" />

      {/* Top row — statistics cards */}
      <div className="kpi-grid">
        <KpiCard icon="bi bi-person-workspace" label="Total Teaching Staff" value={totalStaff} trend="Same" trendDirection="up" color="#ba7517" bg="var(--amber-light)" />
      ...
      <div className="dash-grid-main-side">
        <ChartCard
          title="Teaching Salary Trend"
          subtitle="Teaching staff salary paid per month"
          type="line"
          data={monthlySeries}
          dataKeys={[{ key: "salary", label: "Salary Paid", color: palette.amber }]}
        />
        <ChartCard
          title="Teaching Expenses"
      ...
      <div className="widget-card school-info-full-width" style={{ marginBottom: 0 }}>
        ...
        <p style={{ fontSize: 12.5, color: "var(--text-secondary)", lineHeight: 1.7, margin: 0 }}>
          Teaching staff admissions, salary payments and expenses are managed from this
