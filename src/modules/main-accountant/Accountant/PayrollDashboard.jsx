...
export default function PayrollDashboard({
  ...
  const sorted = useMemo(() => {
    if (!sortable || salarySort === "none") return filtered;
    return [...filtered].sort((a, b) =>
      salarySort === "asc" ? a.total - b.total : b.total - a.total
    );
  }, [filtered, sortable, salarySort]);
          ...
          {classFilterOptions && (
            <select className="form-control" value={classFilter} onChange={(e) => setClassFilter(e.target.value)}>
              ...
              {classFilterOptions.map((c) => (
                <option key={c} value={c}>{/^\d/.test(c) ? `Class ${c}` : c}</option>
              ))}
            </select>
          )}
          {sortable && (
