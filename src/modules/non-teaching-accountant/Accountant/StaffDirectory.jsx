...
// Non-Teaching Staff directory — one card per staff member (mirrors the
...
export default function StaffDirectory() {
  ...
  const designationOptions = useMemo(
    () => [...new Set(nonTeachingRows.map((r) => r.designation))].sort(),
    []
  );

  const totalPayable = nonTeachingRows.reduce((sum, r) => sum + r.salary, 0);
  const totalPending = nonTeachingRows.reduce((sum, r) => sum + Math.max(0, r.salary - r.paid), 0);

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    return nonTeachingRows.filter((r) => {
      const matchesSearch =
  ...
  return (
    <div>
      <PageHeader title="Non-Teaching Staff" subtitle="Manage non-teaching staff, attendance and salary" />
      <LiveClock />

      <div className="row-cards">
        <div className="card stat-card">
          <div className="card-body">
            <h3 className="counter">{nonTeachingRows.length}</h3>
