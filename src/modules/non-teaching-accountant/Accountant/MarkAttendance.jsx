...
// Attendance Management — Non-Teaching module.
...
// Non-Teaching staff member pulled straight from the staff directory.
...
export default function MarkAttendance() {
  ...
  const filteredStaff = useMemo(() => {
    ...
    if (!q) return nonTeachingRows;
    return nonTeachingRows.filter(
      (s) =>
    ...
    <div>
      ...
      <div className="attendance-mgr">
        ...
        <p className="attendance-mgr-hint">
          {isDirty
            ? `${markedCount} of ${nonTeachingRows.length} staff marked — press "Save Attendance" to save everyone's status for ${formatDateLabel(date)}.`
            : savedCount > 0
            ? `Attendance for ${formatDateLabel(date)} is saved — ${savedCount} of ${nonTeachingRows.length} staff recorded.`
        ...
        <div className="attendance-card-panel">
          <div className="attendance-card-panel-head">
            ...
            <div>
              <h4>Non-Teaching Staff Attendance</h4>
              <p>{nonTeachingRows.length} staff · {formatDateLabel(date)}</p>
            </div>
          </div>
              ...
              return (
                <div className={`attendance-person-card ${cardState}`} key={s.key}>
                  <div className="attendance-person-top">
                    <span className="attendance-person-num">{idx + 1}</span>
                    <div className="attendance-person-info">
                      <span className="attendance-person-name">{s.name}</span>
                      <span className="attendance-person-meta">{s.id} · {s.designation} · {s.meta}</span>
                    </div>
