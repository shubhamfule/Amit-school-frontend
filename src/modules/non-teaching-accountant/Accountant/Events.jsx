...
import { schoolEvents as seedEvents, eventThemes } from "./eventsData";
...
export default function Events() {
  ...
  return (
    <div>
      ...
      {modalOpen && (
        <div className="modal-overlay event-modal-overlay" onClick={() => setModalOpen(false)}>
          <div className="modal-box event-modal-box" onClick={(e) => e.stopPropagation()}>
            <div className="modal-head event-modal-head">
              <h5><i className="bi bi-calendar-plus"></i> New Event</h5>
              <button type="button" aria-label="Close" onClick={() => setModalOpen(false)}>&times;</button>
            </div>

            <div className="modal-body event-modal-body">
              <div className="event-form-group event-form-full">
              ...
              <div className="event-form-grid">
                ...
                <div className="event-form-group">
                  ...
                  >
                    {eventThemes.map((t) => (
                      <option key={t.key} value={t.key}>{t.emoji} {t.key === "purple" ? "Sports / Academic" : t.key.charAt(0).toUpperCase() + t.key.slice(1)}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="event-form-group">
                  ...
                  >
                    {["Upcoming", "Scheduled", "Planning"].map((s) => (
            ...
            <div className="modal-foot event-modal-foot">
              <button type="button" className="btn btn-outline" onClick={() => setModalOpen(false)}>
                Cancel
              </button>
              <button type="button" className="btn btn-dark" onClick={addEvent}>
                Save event
              </button>
            </div>
          </div>
