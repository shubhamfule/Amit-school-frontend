...
describe("/api/non-teaching-salary", () => {
  ...
  it("enforces the (roleKey, staffId) unique index with 409", async () => {
    ...
    await request(app).post("/api/non-teaching-salary").send(payload);
    const res = await request(app).post("/api/non-teaching-salary").send(payload);
...
describe("/api/student-fees", () => {
  ...
  it("rejects a duplicate roll with 409", async () => {
    ...
    await request(app).post("/api/student-fees").send(payload);
    const res = await request(app).post("/api/student-fees").send(payload);
    expect(res.status).toBe(409);
  });
});
...
describe("/api/staff-attendance", () => {
  ...
  it("enforces the (staffType, personKey, date) unique index with 409", async () => {
    ...
    await request(app).post("/api/staff-attendance").send(payload);
    const res = await request(app).post("/api/staff-attendance").send(payload);
    expect(res.status).toBe(409);
  });
});
...
describe("/api/library-fines", () => {
  ...
  it("rejects a duplicate fineId with 409", async () => {
    ...
    await request(app).post("/api/library-fines").send(payload);
    const res = await request(app).post("/api/library-fines").send(payload);
    expect(res.status).toBe(409);
  });
});
